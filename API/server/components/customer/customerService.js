...
class CustomerService {
    ...
    async login( formData ) {
        if (formData && formData.email && formData.password) {
            ...
            if (match) {
                ...
                throw new AppError(errorConstants.INVALID_INPUT, errorConstants.USER_NOT_FOUND, true, 404);;
            }
        } else throw new AppError(errorConstants.INVALID_INPUT, errorConstants.USER_NOT_FOUND, true, 404);
    }
    ...
    async deleteCustomer( customer ) {
        customer = await customerModel.destroy({
